...
export default function Producto() {
  ...
  async function savePublication(event) {
    ...
    try {
      ...
      let updateQuery = supabase.from('publicaciones').update({ titulo: draft.titulo.trim(), descripcion: draft.descripcion.trim(), precio: Number(draft.precio) || 0, talla: draft.talla.trim(), categoria: draft.categoria, condicion: draft.condicion, contacto: draft.contacto.trim(), imagen_url: imageUrl, imagenes: imageUrls }).eq('id', producto.id)
      if (!canManage) updateQuery = updateQuery.eq('usuario_id', user.id)
  ...
  async function changeStatus(estado) {
    setError('')
    let updateQuery = supabase.from('publicaciones').update({ estado }).eq('id', producto.id)
    if (profile?.role !== 'admin') updateQuery = updateQuery.eq('usuario_id', user.id)
    const { data, error: updateError } = await updateQuery.select().single()
  ...
  async function deletePublication() {
    ...
    let deleteQuery = supabase.from('publicaciones').delete().eq('id', producto.id)
    if (profile?.role !== 'admin') deleteQuery = deleteQuery.eq('usuario_id', user.id)
  ...
  const canManage = owner || profile?.role === 'admin'
