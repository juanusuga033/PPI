...
export default function Admin() {
  ...
  const [contactForm, setContactForm] = useState({ title: '', subtitle: '', phone: '', email: '', website: '', address: '', image_url: '' })
  ...
  const loadAdminData = useCallback(async () => {
    ...
    if (userError || publicationError || contactError) setError('No fue posible cargar el centro de administración. Verifica que las migraciones de roles y contacto estén aplicadas.')
      ...
      setContactForm(contactData || { title: '', subtitle: '', phone: '', email: '', website: '', address: '', image_url: '' })
    }
    setLoading(false)
  }, [])
  ...
  async function saveContactSettings(event) {
    ...
    try {
      ...
      if (contactImage) {
        validateImage(contactImage, 5 * 1024 * 1024)
        imageUrl = (await uploadPublicationImage(contactImage, profile.id)).url
        uploadedUrl = imageUrl
      }
      const { data, error: updateError } = await supabase.from('contact_settings').update({ ...contactForm, image_url: imageUrl }).eq('id', 1).select().single()
      if (updateError) throw updateError
      if (contactImage && contactForm.image_url?.startsWith('https://')) await removePublicationImage(contactForm.image_url)
      setContactForm(data)
      setContactImage(null)
      setMessage('La información de Contacto fue actualizada.')
    } catch (saveError) {
      if (uploadedUrl) await removePublicationImage(uploadedUrl).catch(() => {})
  ...
  return <div className="admin-page">
    ...
    {loading ? <div className="page-state">Preparando el centro de control...</div> : <>
      <section className="admin-panel admin-contact-panel"><div className="section-heading"><div><span className="eyebrow">Contenido público</span><h2>Sección Contacto</h2></div><span className="admin-pill admin-pill--admin">Solo admins</span></div><p className="admin-panel-note">Actualiza los datos y la foto que aparecen en la página Contacto.</p><form className="admin-contact-form" onSubmit={saveContactSettings}><label>Título<input value={contactForm.title} onChange={event => setContactForm({ ...contactForm, title: event.target.value })} required /></label><label>Subtítulo<input value={contactForm.subtitle} onChange={event => setContactForm({ ...contactForm, subtitle: event.target.value })} required /></label><label>Teléfono<input value={contactForm.phone} onChange={event => setContactForm({ ...contactForm, phone: event.target.value })} required /></label><label>Correo<input type="email" value={contactForm.email} onChange={event => setContactForm({ ...contactForm, email: event.target.value })} required /></label><label>Sitio web<input value={contactForm.website} onChange={event => setContactForm({ ...contactForm, website: event.target.value })} required /></label><label>Ubicación<input value={contactForm.address} onChange={event => setContactForm({ ...contactForm, address: event.target.value })} required /></label><label className="admin-contact-image">Foto del apartado<input type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { const file = event.target.files?.[0]; if (file) setContactImage(file) }} />{contactImage && <small>{contactImage.name}</small>}</label><button className="button primary" disabled={contactSaving}>{contactSaving ? 'Guardando...' : 'Guardar Contacto'}</button></form></section>
